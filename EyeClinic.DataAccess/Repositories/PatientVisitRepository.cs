...
{
    public class PatientVisitRepository : BaseRepository<Model.PatientVisitDto, PatientVisit>, IPatientVisitRepository
    {
        private readonly IMapper _mapper;
        private readonly EyeClinicContext _context;

        public PatientVisitRepository(IMapper mapper, EyeClinicContext context)
            : base(mapper, context)
        {
            _mapper = mapper;
            _context = context;
        ...
        public override async Task<Model.PatientVisitDto> Add(Model.PatientVisitDto modelToAdd)
        {
            var queueIndex = await _context.PatientVisits.AsNoTracking()
                .Where(e => e.VisitDate.Date == modelToAdd.VisitDate.Date)
                .CountAsync();

            modelToAdd.VisitIndex = queueIndex + 1;
            return await base.Add(modelToAdd);
        }

        public async Task<List<Model.PatientVisitDto>> GetVisitsByDate(DateTime date, bool includeStarted)
        {
            var items = await _context.PatientVisits
                    .Include(e => e.Patient)
                    .ThenInclude(e => e.PatientSickStory)
                    .Include(e => e.Patient)
                    .ThenInclude(e => e.PatientDiseases)
                    .ThenInclude(e => e.Disease)
                    .Include(e => e.NotPaymentReason)
                    .Where(e => e.DeletedDate == null
                    && e.VisitDate.Date == date.Date
                                && (includeStarted || e.VisitStatus == 0))
                    .AsNoTracking()
        ...
        public async Task<List<Model.PatientVisitDto>> GetVisits( bool includeStarted)
        {
            var items = await _context.PatientVisits
                    .Include(e => e.Patient)
                    .ThenInclude(e => e.PatientSickStory)
        ...
        public async Task ChangeVisitStatus(int patientVisitId, PatientVisitStatus visitStatus = PatientVisitStatus.Started)
        {
            var visit = await _context.PatientVisits.FindAsync(patientVisitId);
            visit.VisitStatus = (byte)visitStatus;
        ...
        public async Task UpdateVisitNote(int patientVisitId, string note,string pdfpath)
        {
        ...
        public async Task<Model.PatientVisitDto> GetLastVisitByPatientId(int patientId)
        {
            var visit = await _context.PatientVisits
                .AsNoTracking()
                .Where(e => e.PatientId == patientId)
            ...
            return _mapper.Map<Model.PatientVisitDto>(visit);
        }


        public async Task RemoveFromQueue(int id)
        {
            var item = await _context.PatientVisits.FirstOrDefaultAsync(e => e.Id == id);
